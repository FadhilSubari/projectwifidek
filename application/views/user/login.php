  ...
  <div class="row justify-content-center form-login px-5">
    <div class="col-md-5 py-5 p-4 cs-radius shadow cs-color-dsw">
      ...
      <header class="text-center">
        <h2 class="font-weight-bolder cs-color-dsw ">Login</h2>
      </header>
      <form action="<?= base_url('user/login-proses'); ?>" method="POST">
        <div class="form-group">
          <label class="font-weight-bold" for="exampleInputEmail1">Email</label>
          <input type="email" name="email" value="<?= set_value('email', $this->session->flashdata('email')) ?>" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
        </div>
        <div class="form-group">
          <label class="font-weight-bold" for="exampleInputPassword1">Password</label>
          <input type="password" name="password" class="form-control" id="exampleInputPassword1">
        </div>
        <button type="submit" class="btn btn-bg-dsw2 text-white">Masuk</button>
        <a class="btn btn-primary" href="<?= base_url('user/register') ?>">Daftar</a>
        <a class="btn btn-danger" href="<?= base_url('user/lupa-password') ?>">Lupa Password</a>
      </form>
    </div>
  </div>
